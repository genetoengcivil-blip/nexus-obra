...
export default function NexusObraMobileMVP() {
  ...
  // TRAVA DE SEGURANÇA B2B: Verifica autenticação e redireciona caso não tenha obra
  useEffect(() => {
    async function checarSessaoECarregar() {
      ...
      // Verifica se o usuário tem permissão em alguma obra
      ...
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-6 text-center">
        <div className="bg-slate-800 border border-slate-700 p-8 rounded-2xl max-w-md shadow-2xl">
          ...
          <p className="text-slate-400 text-sm mb-6 leading-relaxed">
            Sua conexão com o banco deu certo, mas as tabelas ainda estão vazias.
          </p>
        </div>
      </div>
    );
        ...
        <div className="max-w-md mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <span className="text-[10px] uppercase tracking-widest text-amber-400 font-semibold">Canteiro Mobile v1.4</span>
            </div>
          </div>
          <button 
          ...
          {pavimentos.map((pav) => (
            <div key={pav.id} className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-3 shadow-sm">
              <div className="border-b border-slate-800 pb-2 mb-2.5 flex justify-between items-center">
                <span className="font-bold text-xs text-slate-300">{pav.nome}</span>
              </div>
                ...
                {pav.unidades.map((unid) => {
                  ...
                  return (
                    <button
                      ...
                      className={`py-3 px-3 rounded-lg border font-semibold text-xs transition-all active:scale-95 ${renderizarCorBotao(statusAtual)}`}
                    >
                      <div className="flex justify-between items-center w-full mb-0.5">
                        <span className="text-sm font-bold tracking-tight">{unid.nome}</span>
                        {temFoto && statusAtual === 'concluido' && <Camera className="w-3 h-3" />}
                      </div>
      ...
      {unidadeParaFoto && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-sm rounded-2xl p-6 shadow-2xl">
            <h2 className="text-lg font-bold text-amber-500 mb-4">Prova Material Obrigatória</h2>
            <input type="file" accept="image/*" capture="environment" ref={fileInputRef} onChange={uploadFotoComprovante} className="hidden" />
            <button
              onClick={() => fileInputRef.current?.click()}
              disabled={uploadingFoto}
              className="w-full py-4 bg-emerald-600 text-white font-bold rounded-xl flex items-center justify-center gap-2"
            >
              {uploadingFoto ? 'Enviando...' : 'Abrir Câmera'}
            </button>
          </div>
        </div>
      )}
